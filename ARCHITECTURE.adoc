...
Phronesis is a provably safer language for agentic ethical reasoning.
The repository is a multi-layer project: an Elixir reference
implementation, a Rust compiler workspace, and a formal-verification
layer that gates the metatheory in CI.
...
* *TLA+* (`+tla-consensus.yml+`): model-checks the BFT consensus spec
with TLC.
* *Conformance* (`+conformance.yml+`): runs the language conformance
suite.
...
For governance and maintainers see `+GOVERNANCE.adoc+` and
`+MAINTAINERS.adoc+`.
