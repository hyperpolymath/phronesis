...
Phronesis uses *Ra* (Erlang Raft implementation) for distributed
consensus across multiple nodes. This enables Byzantine fault-tolerant
policy voting in production deployments.
...
[arabic]
. *Consensus.Server* - GenServer wrapper around Ra cluster
. *Consensus.Server.Machine* - Ra state machine for vote processing
. *Stdlib.Consensus* - High-level consensus API with automatic fallback
...
The `Phronesis.Stdlib.Consensus` module automatically uses distributed
...
*Important:* Ensure this directory is persisted in production
deployments.
...
While Raft provides crash fault tolerance, true Byzantine fault
tolerance requires:

[arabic]
. Minimum 3f+1 nodes (f = Byzantine nodes)
. Additional validation layers (crypto signatures, etc.)
. This is Phase 3 future work
...
See `deploy/kubernetes/consensus-statefulset.yaml` for a
...
. Verify node names and cookies match
. Check firewall rules (EPMD port 4369, distribution ports)
...
* link:../SPEC.core.scm[SPEC.core.scm] - Phronesis consensus
specification
