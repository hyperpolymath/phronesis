...
* Autocomplete for Std.BGP._, Std.RPKI._, etc.
...
[arabic]
. *Syntax highlighting* (2-3 days) ← Quick win
. *Testing framework* (1 week) ← Essential
. *LSP server* (2-3 weeks) ← Game changer
. *Debugger* (1-2 weeks)
. *Profiler* (1 week)
. *Doc generator* (1 week)
