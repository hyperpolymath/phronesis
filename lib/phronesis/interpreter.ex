...
defmodule Phronesis.Interpreter do
  ...
  defp resolve_module(["Std", module_name | _rest] = path) do
    ...
    case Code.ensure_loaded(elixir_module) do
      {:module, _mod} ->
        # Return a module wrapper that allows calling functions
        {:ok, %{
          name: Enum.join(path, "."),
          elixir_module: elixir_module,
          call: fn args -> {:error, "use resolve_builtin_module instead"} end
        }}
      _ ->
        :error
    end
  ...
  # Built-in module implementations - wired to actual stdlib

  # Std.RPKI module
  defp resolve_builtin_module(["Std", "RPKI", "validate"], [route]) do
    {:ok, Phronesis.Stdlib.RPKI.validate(route)}
  end

  defp resolve_builtin_module(["Std", "RPKI", "check_origin"], [asn, roa_asn]) do
    {:ok, Phronesis.Stdlib.RPKI.check_origin(asn, roa_asn)}
  end

  defp resolve_builtin_module(["Std", "RPKI", "validation_status"], [status]) do
    {:ok, Phronesis.Stdlib.RPKI.validation_status(status)}
  end

  # Std.BGP module
  defp resolve_builtin_module(["Std", "BGP", "extract_as_path"], [route]) do
    {:ok, Phronesis.Stdlib.BGP.extract_as_path(route)}
  end

  defp resolve_builtin_module(["Std", "BGP", "get_origin"], [route]) do
    {:ok, Phronesis.Stdlib.BGP.get_origin(route)}
  end

  defp resolve_builtin_module(["Std", "BGP", "path_length"], [route]) do
    {:ok, Phronesis.Stdlib.BGP.path_length(route)}
  end

  defp resolve_builtin_module(["Std", "BGP", "validate_route"], [route]) do
    {:ok, Phronesis.Stdlib.BGP.validate_route(route)}
  end

  defp resolve_builtin_module(["Std", "BGP", "is_private_asn"], [asn]) do
    {:ok, Phronesis.Stdlib.BGP.is_private_asn?(asn)}
  end

  defp resolve_builtin_module(["Std", "BGP", "is_bogon_asn"], [asn]) do
    {:ok, Phronesis.Stdlib.BGP.is_bogon_asn?(asn)}
  end

  # Std.Consensus module
  defp resolve_builtin_module(["Std", "Consensus", "vote"], [action, agents, threshold]) do
    case Phronesis.Stdlib.Consensus.vote(action, agents, threshold) do
      {:ok, result, votes} -> {:ok, %{result: result, votes: votes}}
      error -> error
    end
  end

  defp resolve_builtin_module(["Std", "Consensus", "count_approvals"], [votes]) do
    {:ok, Phronesis.Stdlib.Consensus.count_approvals(votes)}
  end

  # Std.Temporal module
  defp resolve_builtin_module(["Std", "Temporal", "now"], []) do
    {:ok, Phronesis.Stdlib.Temporal.now()}
  end

  defp resolve_builtin_module(["Std", "Temporal", "is_expired"], [timestamp, duration]) do
    {:ok, Phronesis.Stdlib.Temporal.is_expired(timestamp, duration)}
  end

  defp resolve_builtin_module(["Std", "Temporal", "within_window"], [start_time, end_time]) do
    {:ok, Phronesis.Stdlib.Temporal.within_window(start_time, end_time)}
  end

  defp resolve_builtin_module(["Std", "Temporal", "parse"], [timestamp_str]) do
    Phronesis.Stdlib.Temporal.parse(timestamp_str)
  end

  defp resolve_builtin_module(["Std", "Temporal", "format"], [datetime]) do
    {:ok, Phronesis.Stdlib.Temporal.format(datetime)}
  end

  defp resolve_builtin_module(["Std", "Temporal", "duration"], [start_time, end_time]) do
    {:ok, Phronesis.Stdlib.Temporal.duration(start_time, end_time)}
  end

  defp resolve_builtin_module(["Std", "Temporal", "to_unix"], [datetime]) do
    {:ok, Phronesis.Stdlib.Temporal.to_unix(datetime)}
  end

  defp resolve_builtin_module(["Std", "Temporal", "from_unix"], [unix_timestamp]) do
    Phronesis.Stdlib.Temporal.from_unix(unix_timestamp)
  end
