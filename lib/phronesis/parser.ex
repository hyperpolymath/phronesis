...
defmodule Phronesis.Parser do
  ...
  @doc """
  Parse with error recovery. Collects multiple diagnostics and returns
  a partial AST containing all successfully parsed declarations.

  Returns `{:ok, ast, diagnostics}` where `diagnostics` is a list of
  error tuples. An empty diagnostics list means parsing was clean.

  ## Examples

      iex> tokens = Phronesis.Lexer.lex("CONST x = 1\\nBADTOKEN\\nCONST y = 2")
      iex> {:ok, ast, diagnostics} = Phronesis.Parser.parse_recovering(tokens)
      iex> length(ast) >= 1
      true
  """
  @spec parse_recovering(tokens()) :: {:ok, list(), list()}
  def parse_recovering(tokens) when is_list(tokens) do
    {ast, diagnostics} = parse_program_recovering(tokens, [], [])
    {:ok, Enum.reverse(ast), Enum.reverse(diagnostics)}
  end

  # Program ::= { Declaration } — with error recovery
  defp parse_program_recovering([{:eof, _, _, _} | _], acc, diags) do
    {acc, diags}
  end

  defp parse_program_recovering([], acc, diags) do
    {acc, diags}
  end

  defp parse_program_recovering(tokens, acc, diags) do
    case parse_declaration(tokens) do
      {:ok, decl, rest} ->
        parse_program_recovering(rest, [decl | acc], diags)

      {:error, reason} ->
        # Record the diagnostic and synchronize to next declaration
        rest = synchronize_to_declaration(tokens)
        parse_program_recovering(rest, acc, [reason | diags])
    end
  end

  # Skip tokens until we find one that could start a new declaration
  # (POLICY, IMPORT, CONST) or hit EOF.
  defp synchronize_to_declaration([{:eof, _, _, _} | _] = tokens), do: tokens
  defp synchronize_to_declaration([]), do: []

  defp synchronize_to_declaration([{type, _, _, _} | _] = tokens)
       when type in [:policy, :import, :const] do
    tokens
  end

  defp synchronize_to_declaration([_ | rest]) do
    synchronize_to_declaration(rest)
  end
