...
[[31-canonical-forms-lemma]]
==== 3.1 Canonical Forms Lemma

*Lemma 3.1 (Canonical Forms):*

[arabic]
. If v is a closed value of type Int, then v = n for some integer n
. If v is a closed value of type Bool, then v = true or v = false
. If v is a closed value of type List(τ), then v = [v₁, ..., vₙ]
. If v is a closed value of type Record\{l₁:τ₁,...}, then v = \{l₁:v₁,...}
...
[arabic]
. e →* v for some value v with ∅ ⊢ v : τ, or
. e diverges (impossible in Phronesis by Termination theorem)
...
[[42-type-inference-is-decidable]]
...
. Wright, A., & Felleisen, M. (1994). _A Syntactic Approach to Type Soundness_.
. Harper, R. (2016). _Practical Foundations for Programming Languages_. Cambridge.
